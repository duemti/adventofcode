...
class	IntcodeProcessor
{
	...
	public function	__construct(array $program, array $halt_IO)
	{
	...
	public function	restart(array $program = null, int $ip = 0)
	{
		$this->ip = $ip;
	...
	public function	dump_state(): array
	{
		return ['memory' => $this->memory, 'ip' => $this->ip];
	}
		...
		while ( !$this->end && !$this->suspend )
		{
			...
			switch (intval(substr($opcode, -2)))
			{
				...
				case 3: // Opcode for requesting single integer.
					if ($this->halt_IO['input'] && empty($this->input))
						$this->halt("input");
					else
